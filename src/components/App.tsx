...
type RouteState =
  | { page: 'purrdle'; mode: GameMode; wordEntry: WordEntry }
  | { page: 'mansion' }
...
function parseHash(): RouteState {
  ...
  if (hash === 'random') {
    ...
    return { page: 'purrdle', mode: 'random', wordEntry: entry }
  }
  ...
  if (hash.startsWith('w/')) {
    ...
    if (index !== null) {
      ...
      if (entry) {
        return { page: 'purrdle', mode: 'specific', wordEntry: entry }
      }
    }
    // Invalid ID — fall through to daily
  }

  return { page: 'purrdle', mode: 'daily', wordEntry: getTodayWord() }
}
...
export default function App() {
  ...
  return (
    <div className="h-[100dvh] bg-purple-50 text-gray-800 flex flex-col paw-bg relative overflow-hidden">
      {route.page !== 'mansion' && <CatBackground current={bg.current} fading={bg.fading} />}
