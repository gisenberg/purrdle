...
function loadState(key: string, target: string): SavedState | null {
  try {
    const raw = localStorage.getItem(`purrdle-${key}`)
    if (!raw) return null
...
function saveState(key: string, state: SavedState) {
  localStorage.setItem(`purrdle-${key}`, JSON.stringify(state))
}
...
export function useGame(wordEntry: WordEntry, mode: GameMode) {
  ...
  // When a hint reveals a new position, also reveal all other occurrences of the
  // same letter so duplicate letters (e.g. both b's in "kibble") are shown together.
  const revealHintPosition = useCallback((pos: number) => {
    setHintedPositions((prev) => {
      ...
      for (let i = 0; i < target.length; i++) {
        if (target[i] === letter && !revealedPositions.has(i)) {
          next.add(i)
        }
      }
      return next
  ...
  const addLetter = useCallback(
    (letter: string) => {
      ...
      // If the user types a letter that matches a revealed/hinted position
      // immediately before the cursor, consume it silently. This handles the
      // case where the user sees a pre-filled vowel or hinted letter on screen
      // and types it thinking they need to. Only consume for revealed positions
      // — never block input at non-revealed slots (e.g. duplicate consonants).
      if (visualPos > 0 && allRevealedPositions.has(visualPos - 1) && target[visualPos - 1] === l) {
        // But if the current (non-revealed) slot also needs the same letter,
        // don't consume — the user is typing a legitimate duplicate.
        if (target[visualPos] !== l) return
      }
  ...
  const generateShareText = useCallback(() => {
    ...
    const shareUrl = `https://gisenberg.github.io/purrdle/#/w/${encodedId}`

    const dayIndex = getDayIndex()
    const label = isDaily ? `Purrdle #${dayIndex}` : 'Purrdle'
